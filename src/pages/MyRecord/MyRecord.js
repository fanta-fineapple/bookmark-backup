...
const MyRecord = () => {
  // const [value, setValue] = useState(moment("06-2022", "MM-YYYY"));
  const [value, setValue] = useState(() => getMonth());
  // const [value, setValue] = useState(moment());

  const [bookList, setBookList] = useState([]);
  const [tab, setTab] = useState(() => {
    return Number(window.sessionStorage.getItem("view")) || 0;
  });
  ...
  return (
    <MyRecordWrap>
      <ViewIcon>
        <div onClick={() => clickTab(0)}>
          <FiCalendar className="icon" />
        </div>
        <div onClick={() => clickTab(1)}>
      ...
      {tab === 0 ? (
        <>
          <Calendar
            ...
            bookList={bookList}
          />
          <AddBook onClick={() => navigate("/search")}>
            <BsPlusCircleFill className="addBook" />
          </AddBook>
        </>
      ) : (
